...
Blockly.Python['driss_opencv2_add_trackbar'] = function (block) {
  ...
  Blockly.Python.functionNames_['on_' + trackbar_name_sans_espaces + 'cursorSlide'] = "\n" +
    "def on_" + trackbar_name_sans_espaces + "cursorSlide(val) :\n" +
    "   global " + var_name + "\n" +
    "   " + var_name + " = val\n" +
    "   cv2.setTrackbarPos('" + trackbar_name_sans_espaces + "', '" + windows + "', " + var_name + ")";

  var code = 'cv2.createTrackbar("' + trackbar_name + '", "' + windows + '" , ' + var_name + ', ' + max + ', on_' + trackbar_name_sans_espaces + 'Change)\n';
  return code;
};
...
Blockly.Python['driss_opencv2_ocr_sur_plaque_immatriculation'] = function (block) {
  ...
  '        for elem in detection :\n' +
  '            if(len(__immatriculation__) < len(elem[1])) :\n' +
  '                __immatriculation__ = elem[1]\n' +
  '                __tauxDeConfiance__ = f"{elem[2] * 100:.2f}"\n' +
  
  '        #print("Immatriculation detecté par OCR : ",__immatriculation__)\n' +
  '        #print("tauxConfiance : ",__tauxDeConfiance__)\n'+
  '        return(__immatriculation__.upper(), __tauxDeConfiance__)\n' ;
...
Blockly.Python['driss_opencv2_load_all_photos_from_folder'] = function (block) {
  ...
  Blockly.Python.functionNames_["chargerLesImagesDuDossier"] = '#Charger toutes les photos du dossier\n'+
  'def chargerLesImagesDuDossier(dossier) :\n' +
  ...
  Blockly.Python.functionNames_["identifierVisagesSurPhotos"] = '#Identifier les visages sur les photos et les associer à des noms d\'utilisateurs (nom d\'utilisateur = nom de fichier)\n'+
  'def identifierVisagesSurPhotos(dossier) :\n' +
  ...
  var code = '';
  code += 'chargerLesImagesDuDossier('+folder+')\n';
  code += 'identifierVisagesSurPhotos('+folder+')\n'; 
...
Blockly.Python['driss_opencv2_nom_visage_reconnu'] = function (block) {
  ...
  var cb_mark_in_Picture = block.getFieldValue('CB_MARK_IN_PICTURE') === 'TRUE';

  Blockly.Python.definitions_["import_CV2"] = "import cv2 as cv2";
  Blockly.Python.definitions_["import_face_recognition"] = "import face_recognition";
  Blockly.Python.definitions_["import_math"] = "import math";

  Blockly.Python.functionNames_["getNomVisageSurPhoto"] = '#Retourne le nom du visage identifié sur la photo\n'+
  'def getNomVisageSurPhoto(photo, cb_mark_in_Picture) :\n' +
  '    global __known_face_encodings__, __known_face_names__\n'+
  '    imageAvecVisageAIdentifier = photo\n' +
  '    # Trouver dans l\'image inconnue tous les visages encodées (connus car dans le dossier)\n' +
  '    face_locations = face_recognition.face_locations(imageAvecVisageAIdentifier)\n' +
  '    face_encodings = face_recognition.face_encodings(imageAvecVisageAIdentifier, face_locations)\n' +
  
  '    \n'+
  '    mem_aire = 0\n'+
  '    nom = ""\n'+
  '    (match_top, match_right, match_bottom, match_left) =(0,0,0,0)\n'+
  '    # Pour chaque face reconnue sur la photo\n' +
  '    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):\n' +
  '        aire = (right-left) * (bottom-top)\n' +
  ...
  '            \n' +
  '            # Afficher l\'image\n' +
  '            cv2.imshow("Photo issue de la camera du parking", photo)\n' +
  '            return(nom)\n' +
  '        else:\n' +
  '            return(nom)\n' +
  '    return("Inconnu")' ;
  ...
  var code = '';
  if (cb_mark_in_Picture)
    code = 'getNomVisageSurPhoto('+photo+', True)';
  else 
    code = 'getNomVisageSurPhoto('+photo+', False)';
